 ...
 */
// @ts-expect-error — Vue 2 default export (constructor) vs Vue 3 types mismatch
import VueConstructor from 'vue'
import { Injector, InjectionToken, type Token } from './injector'

// ─── 工具类型 ──────────────────────────────────────────────

type ClassToken<T> = { new (...args: any[]): T }
type Resolved<T> = T extends InjectionToken<infer V> ? V
  : T extends ClassToken<infer V> ? V
  : never

/** 将 PascalCase / SCREAMING_SNAKE 转为 camelCase */
type CamelCase<S extends string, FromSnake extends boolean = false> =
  S extends `${infer A}_${infer B}`
    ? `${Lowercase<A>}${Capitalize<CamelCase<B, true>>}`
    : FromSnake extends true
...
export const ServicePlugin = {
  install(
    VueInstance: typeof VueConstructor,
 ...
 * ```
 */
export function mapInject<T extends Record<string, Token<any>>>(
  tokens: T,
): ComputedGetters<MapInjectResult<T>> {
  const result = {} as any
  for (const key of Object.keys(tokens)) {
    const camelKey = toCamelCase(key)
    const token = tokens[key]
    result[camelKey] = function (this: any) {
